...
export default function ActivitySelector({ activities, onChange, showCustomAlert }: ActivitySelectorProps) {
  ...
  const handlePhotoUpload = async (id: string, files: FileList | null) => {
    ...
    if (remainingSlots <= 0) {
      if (showCustomAlert) {
        showCustomAlert('Limit Unggah Foto', 'Batas maksimal unggah foto untuk setiap rincian kegiatan adalah 10 foto!');
      } else {
    ...
    for (const file of filesToUpload) {
      try {
        ...
        if (showCustomAlert) {
          showCustomAlert('Gagal Mengunggah', err.message || 'Gagal memproses berkas gambar');
        } else {
