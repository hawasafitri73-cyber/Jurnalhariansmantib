...
import { User, ClipboardList, Settings2, Sliders, Eye, Scale, AlertCircle } from 'lucide-react';

export default function App() {
  // --- States with Robust Lazy Initialization from localStorage ---
  const [profile, setProfile] = useState<TeacherProfile>(() => {
    try {
      const stored = localStorage.getItem('lkh_teacher_profile');
      if (stored) {
      ...
      console.error('Gagal membaca lkh_teacher_profile dari localStorage:', err);
    }
  ...
  const [activities, setActivities] = useState<ActivityItem[]>(() => {
    try {
      const stored = localStorage.getItem('lkh_teacher_activities_v4');
      if (stored) {
        return JSON.parse(stored);
      }
    } catch (err) {
      console.error('Gagal membaca lkh_teacher_activities_v4 dari localStorage:', err);
    }
  ...
  const [settings, setSettings] = useState<PrintSettings>(() => {
    try {
      const stored = localStorage.getItem('lkh_print_settings');
      if (stored) {
        return JSON.parse(stored);
      }
    } catch (err) {
      console.error('Gagal membaca lkh_print_settings dari localStorage:', err);
    }
    return {
      showOnlyActive: true,
      gridCols: 2,
  ...
  // --- States for Custom Confirmation and Alert Dialogs (bypasses iframe restrictions) ---
  ...
  const showCustomConfirm = (title: string, message: string, onConfirm: () => void, confirmText = 'Ya, Lanjutkan', cancelText = 'Batal') => {
    setModalConfig({
      isOpen: true,
      title,
      message,
  ...
  useEffect(() => {
    try {
      ...
      try {
        localStorage.setItem('lkh_teacher_activities_v4', JSON.stringify(activities));
      } catch (errQuota) {
        ...
        localStorage.setItem('lkh_teacher_activities_v4', JSON.stringify(textOnlyActivities));
      }
    } catch (err) {
  ...
  const handleExportBackup = () => {
    try {
      ...
      showCustomAlert('Ekspor Gagal', 'Gagal mengekspor berkas data atau format data salah.');
    }
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (payload.profile && payload.activities) {
          ...
          showCustomAlert('Impor Berhasil', 'Data LKH berhasil diimpor!');
        } else {
          showCustomAlert('Format Salah', 'Format berkas backup JSON tidak sesuai.');
        }
      } catch (err) {
        showCustomAlert('Impor Gagal', 'Gagal membaca berkas. Pastikan format berkas sesuai.');
      }
    };
  ...
  const handleResetActivities = () => {
    showCustomConfirm(
      'Setel Ulang Kegiatan',
      'Apakah Anda yakin ingin menyetel ulang daftar kegiatan ke bawaan pabrik? Customisasi kegiatan Anda akan hilang.',
      () => {
        setActivities(DEFAULT_ACTIVITIES);
        setProfile(prev => ({ ...prev, isKepalaSekolah: false }));
        showCustomAlert('Berhasil', 'Daftar kegiatan berhasil disetel ulang ke default!');
      }
    );
  };

  const handleLoadKepalaSekolah = () => {
    showCustomConfirm(
      'Muat Template Kepala Sekolah',
      'Apakah Anda yakin ingin memuat Template Kegiatan harian Kepala Sekolah? Seluruh list kegiatan akan digantikan dengan template Khusus Kepala Sekolah (Supervisi & Manejerial).',
      () => {
        setActivities(KEPALA_SEKOLAH_ACTIVITIES);
        setProfile(prev => ({ 
          ...prev, 
          isKepalaSekolah: true, 
          tugasTambahan: 'Kepala Sekolah', 
          mataPelajaran: 'Kepala Sekolah',
          namaGuru: prev.namaKepalaSekolah || 'Yakob Simson Barthimeus S.Pd., M.Pd., MM',
          nip: prev.nipKepalaSekolah || '19781203200701 1 004',
          jabatan: prev.jabatanKepalaSekolah || 'Pembina Utama Muda, IV/c'
        }));
        showCustomAlert('Berhasil', 'Template kegiatan harian Kepala Sekolah berhasil dimuat dan data profil kepala sekolah otomatis terisi!');
      }
    );
  };
  ...
  const handleResetAllData = () => {
    showCustomConfirm(
      ...
      'Apakah Anda yakin ingin menyetel ulang SEMUA data (termasuk profil guru, kepala sekolah, beban kerja, dan daftar kegiatan harian) kembali ke draf contoh SMA Negeri 3 Bombana?',
      () => {
        setProfile(DEFAULT_TEACHER_PROFILE);
        setActivities(DEFAULT_ACTIVITIES);
        localStorage.removeItem('lkh_teacher_profile');
        localStorage.removeItem('lkh_teacher_activities_v4');
        showCustomAlert('Berhasil', 'Semua data berhasil disetel ulang ke draf contoh Jurnal Harian Guru!');
      }
    );
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-[1600px] w-full mx-auto p-4 md:p-6 grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <section className="xl:col-span-5 space-y-5 no-print flex flex-col max-h-[calc(100vh-120px)] overflow-y-auto pr-1">
          ...
          <div className="flex-1">
            ...
            {activeTab === 'kegiatan' && (
              <div className="animate-fade-in">
                <ActivitySelector activities={activities} onChange={setActivities} showCustomAlert={showCustomAlert} />
              </div>
            )}
            ...
            {activeTab === 'kustomizer' && (
              <div className="animate-fade-in">
                <ActivityCustomizer
                  ...
                  onLoadKepalaSekolah={handleLoadKepalaSekolah}
          ...
          <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-5 space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              {/* Option 3: Signature & Seal checkboxes */}
              <div className="pt-3 border-t border-dashed border-slate-100 space-y-2">
                <label className="font-semibold text-slate-700 block text-xs">Tanda Tangan & Stempel Resmi</label>
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={!!settings.showDigitalSignature}
                      onChange={(e) => setSettings({ ...settings, showDigitalSignature: e.target.checked })}
                      className="rounded border-slate-300 text-sky-600 focus:ring-sky-500 w-4 h-4 cursor-pointer"
                    />
                    <span className="text-slate-600 text-[11px] font-medium">Tampilkan TTD Digital Otomatis (Cursive)</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-slate-300 text-sky-600 focus:ring-sky-500 w-4 h-4 cursor-pointer"
                    />
                    <span className="text-slate-600 text-[11px] font-medium">Sematkan Stempel Kiri SMAN 3 Bombana</span>
                  </label>
                </div>
              </div>
      ...
      {/* 4. Elegant Custom Confirmation and Alert Modal dialogs (Bypassing secure iframe limits) */}
      {modalConfig.isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4 animate-fade-in no-print">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden transform scale-100 transition-all">
            <div className="p-6 space-y-4">
              <div className="flex items-start gap-3.5">
                <div className="p-2.5 bg-sky-50 rounded-full text-sky-600 shrink-0">
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed white-space-pre-wrap">
                    {modalConfig.message}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-slate-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 border-t border-slate-100">
              {!modalConfig.isAlert && (
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-bold rounded-lg text-xs cursor-pointer text-center"
                >
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-2 text-white font-extrabold rounded-lg text-xs cursor-pointer text-center bg-sky-600 hover:bg-sky-700"
              >
